...
def get_or_create_social_user(db: Session, email: str, full_name: str, provider: str, social_id: str, avatar_url: str = None):
    # 1. 先用 Email 找 (防止重複註冊)
    user = db.query(User).filter(User.email == email).first()
    
    if user:
        # 用戶已存在 → 帳號合併
        # 更新社交登入資訊（如果原本是 Email 註冊）
        updated = False
        
        # 如果原本是 local 註冊，記錄社交帳號綁定
