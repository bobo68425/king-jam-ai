...
from app.services.picwish_service import picwish_service
...
class RemoveBackgroundRequest(BaseModel):
    ...
    return_type: int = 2                  # 1=URL, 2=Base64（前端使用 Base64 更方便）
    use_async: bool = False               # 是否使用非同步模式


class RemoveBackgroundResponse(BaseModel):
    """去背回應模型"""
    success: bool
    image: str           # 去背後的圖片（URL 或 Base64）
...
async def remove_background(
    ...
    - return_type: 1=返回 URL, 2=返回 Base64（預設）
    - use_async: 是否使用非同步模式（大圖片建議使用）
    """
    ...
    try:
        if request.use_async:
            result = await picwish_service.remove_background_async(
            ...
            result = await picwish_service.remove_background_sync(
                image_base64=request.image_base64,
                image_url=request.image_url,
                output_type=request.output_type,
                return_type=request.return_type,
...
async def check_api_status():
    ...
    return {
        "picwish_configured": bool(api_key),
        "message": "PicWish API 已設定" if api_key else "請設定 PICWISH_API_KEY 環境變數"
    }
