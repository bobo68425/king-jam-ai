...
class Expense(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    item_name = Column(String, nullable=False)  # 項目名稱
    amount = Column(Numeric(12, 2), nullable=False)  # 金額
    category = Column(String, default="other")  # 分類 (cloud, gpu, ad, fine-tune, other)
    description = Column(Text, nullable=True)  # 說明
    expense_date = Column(DateTime(timezone=True), server_default=func.now())  # 支出日期
