...
class TikTokPlatform(BasePlatform):
    ...
    def create_config(cls) -> PlatformConfig:
        ...
        return PlatformConfig(
            ...
            scopes=[
                ...
                "user.info.stats",      # 用戶統計數據（粉絲數、按讚數等）
                "video.list",           # 獲取影片列表
                "video.publish",        # 發布影片
                "video.upload"          # 上傳影片
            ],
    ...
    async def get_video_list(self, access_token: str, max_count: int = 20) -> List[Dict[str, Any]]:
        """
        獲取用戶影片列表
    ...
    async def get_video_insights(self, access_token: str, video_ids: List[str]) -> Dict[str, Dict[str, Any]]:
        """
        獲取特定影片的詳細成效數據
        ...
        async with aiohttp.ClientSession() as session:
            ...
            data = {"filters": {"video_ids": video_ids}}
