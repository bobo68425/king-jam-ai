...
TONE_STYLES = {
    ...
    "faith": {
        "name": "信仰靈性",
        "voice": "溫柔堅定、充滿盼望，帶有屬靈的深度，以基督教信仰為核心",
        "structure": "真理闡述、生命見證、信仰應用",
        "audience": "基督徒、慕道友、追尋信仰的人",
        "examples": "如靈修文章、生命見證、教會信息"
    },
}
...
# 基督教信仰用語指導（背景 Prompt）
# ============================================================
FAITH_BACKGROUND_PROMPT = """
## 🙏 信仰用語指導（基督教用語為主）

當生成與信仰、靈性相關的內容時，請優先使用以下基督教用語：

### 至高存在
✅ 使用：天父、主（Lord）、三位一體的神、上帝、耶和華
❌ 避免：宇宙、源頭（Source）、造物主（非人格化的用法）

### 人的地位
✅ 使用：受造物、神的兒女、罪人（需要救贖的人）
❌ 避免：神性的一部分、共同創造者、高我

### 困境定義
✅ 使用：罪（Sin）、墮落、老我、破碎、與神的關係斷裂
❌ 避免：低頻率、能量阻塞、小我執著、幻象

### 轉變動力
✅ 使用：恩典（Grace）、救贖、信心、神的愛、十字架的大能
❌ 避免：覺醒（Awakening）、提升、顯化、吸引力法則

### 改變過程
✅ 使用：悔改（Repent）、重生、成聖、心意更新、效法基督
❌ 避免：清理、淨化（能量意義）、對齊（宇宙能量意義）

### 日常實踐
✅ 使用：禱告、團契、尋求神旨、交託、讀經、敬拜
❌ 避免：冥想（東方宗教意義）、連結宇宙、順流、吸引力法則

### 最終目的
✅ 使用：榮耀神、神國度降臨、永生、與神同在
❌ 避免：自我實現、靈魂進化、揚升、豐盛法則
...
"""
    ...
    # 如果是信仰靈性風格，加入基督教用語指導
    if tone == "faith":
        instructions = FAITH_BACKGROUND_PROMPT + "\n" + instructions
