...
async def generate_image_with_gemini(topic: str, quality: str, tone: str, platform: str = "instagram", reference_image_base64: Optional[str] = None, image_prompt: Optional[str] = None) -> str:
    ...
    # 構建優化的 prompt - 禁止文字的指令放在最前面
    prompt = f"""[CRITICAL RULE - NO TEXT]: Generate a PURELY VISUAL image. DO NOT include ANY text, words, letters, numbers, typography, titles, captions, labels, watermarks, logos, signs, or any readable characters in ANY language. The image must be 100% text-free.
...
⚠️ 重要：直接輸出貼文內容本身，不要任何前言、說明、標題、分隔線或「這是文案」之類的話。就像直接貼到社群媒體上那樣。"""
