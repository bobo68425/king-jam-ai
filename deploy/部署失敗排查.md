...
若部署完成後在 https://kingjam.app 登入時出現「無法登入」或「登入失敗」：

| 檢查項 | 說明 |
|--------|------|
| **錯誤訊息** | 登入頁已區分「無法連線至伺服器」與「帳號或密碼錯誤」。若看到「無法連線至伺服器」，代表瀏覽器打不到後端 API。 |
| **前端 API 網址** | 前端建置時需帶入 `NEXT_PUBLIC_API_URL=https://api.kingjam.app`。Workflow 已設定；若曾改過 Dockerfile 或 build-arg，請確認有傳入。 |
| **後端 CORS** | 後端 `main.py` 的 `allow_origins` 需包含 `https://kingjam.app`（已內建）。若改過網域，請一併加入。 |
| **後端是否正常** | 瀏覽器開 https://api.kingjam.app/health 應回 200。若打不開，表示後端未啟動或網域/負載平衡有問題。 |
| **Google / Facebook 登入** | 若用社群登入，請在 Google Cloud Console、Facebook 開發者後台將 **授權的重新導向 URI** 設為 `https://kingjam.app/auth/callback/google`、`https://kingjam.app/auth/callback/facebook`（不可用 localhost）。 |
| **環境變數** | 前端需有 `NEXT_PUBLIC_GOOGLE_CLIENT_ID`、`NEXT_PUBLIC_FACEBOOK_APP_ID`（建置時寫入）。若用 Cloud Run 建置，需在 workflow 的 build-arg 或 Dockerfile ARG 傳入。 |
